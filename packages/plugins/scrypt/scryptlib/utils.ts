...
import { decode } from 'npm:@jridgewell/sourcemap-codec'
import { bsv } from 'scrypt-ts'
import { parseChunked, stringifyStream } from 'npm:@discoveryjs/json-ext'

import type { ABIEntity, LibraryEntity, OpCode } from './compilerWrapper.ts'
import { compileAsync } from './compilerWrapper.ts'
import type { AbstractContract, CompileResult, Script, StructEntity, SupportedParamType } from './internal.ts'
import { ScryptType, compile, findCompiler, getValidatedHexString } from './internal.ts'
import { arrayTypeAndSizeStr, isGenericType, parseGenericType } from './typeCheck.ts'
...
export function asm2int(str: string): number | string {
  switch (str) {
    ...
    case 'OP_15':
    case 'OP_16':
      return Number.parseInt(str.replace('OP_', ''))
    default: {
...
export function subscript(index: number, arraySizes: Array<number>): string {
  if (arraySizes.length == 1) {
    ...
    const offset = subArraySizes.reduce((acc, val) => { return acc * val }, 1)
...
export async function compileContract(opts: { code: string; id: string }, options?: {
  out?: string
  sourceMap?: boolean
  artifact?: boolean
