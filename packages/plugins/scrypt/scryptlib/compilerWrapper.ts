...
import * as rimraf from 'npm:rimraf'
import JSONbig from 'npm:json-bigint'
import type { Artifact, TypeResolver } from './internal.ts'
import { CURRENT_CONTRACT_ARTIFACT_VERSION, buildTypeResolver, findCompiler, hash160, md5, path2uri, resolveConstValue } from './internal.ts'
...
export async function compile(
  ...
  const sourcePath = opts.id

  const srcDir = sourcePath.replace('.ts.scrypt', '')
  ...
  // const curWorkingDir = settings.cwd || srcDir;
  // settings.cwd = "/Users/X/Documents/GitHub/kitto"

  settings = Object.assign({}, defaultCompilingSettings, settings)

  const sourceContent = opts.code

  // const maxBuffer = settings.stdout ? 1024 * 1024 * 100 : 1024 * 1024;
  settings = Object.assign({}, defaultCompilingSettings, settings)
  const cmds = settings2cmd(sourcePath, settings)

  const argsAll = cmds.split(' ').filter(a => !!a)

  let outDir = argsAll[argsAll.length - 1]
  outDir = outDir.slice(1, outDir.length - 1)

  argsAll[argsAll.length - 1] = outDir

  // const cmd = argsAll[0]
  // const args = argsAll.slice(1).filter(a => !!a)
  // const args=  `compile --hex --ast -r -o ${srcDir}`.split(" ")
  const args = argsAll.slice(1)

  const command = new Deno.Command("scryptc", {
    // args: `compile --hex --ast -r -o "/Users/X/Documents/GitHub/kitto/packages/plugins/scrypt/out/foo.scrypt-c79153ea8a"`.split(" "),
    // args,

    cwd: path.dirname(opts.id),

    args: `compile --hex --ast -r -o ${outDir}`.split(' '),
    // args: `compile --asm --hex --ast -r -o "/Users/X/Documents/GitHub/kitto/dist/mast.ts.scrypt-f1146300cb"`.split(" "),
    stdin: 'piped',
  ...
  const stdin = server.stdin.getWriter()

  await stdin.write(encoder.encode(sourceContent))
  ...
  // const r = await server.output()

  // r.stdou

  // const output = decoder.decode( r.stdout)

  // await Deno.writeTextFile("/Users/X/Documents/GitHub/kitto/packages/plugins/scrypt/scryptlib/out.txt", output)

  return handleCompilerOutput(sourcePath, settings, outDir, md5(sourceContent))
}
import fs from "node:fs"
export async function handleCompilerOutput(
  sourcePath: string,
  settings: CompilingSettings,
  outputDirX: string,
  md5: string,
  ...
  try {
    ...
    // result.hex = await Deno.readTextFile(`${outputDir}/stdin_hex.txt`)
...
function parserASM(result: CompileResult, asmObj: any, settings: CompilingSettings, srcDir: string, sourceFileName: string) {
  ...
  result.asm = asmObj.output.map((item) => {
    ...
    if (match && match.groups) {
      const fileIndex = Number.parseInt(match.groups.fileIndex)
      ...
      const pos: Pos | undefined = sources[fileIndex]
        ? {
            ...
            line: sources[fileIndex] ? Number.parseInt(match.groups.line) : undefined,
            endLine: sources[fileIndex] ? Number.parseInt(match.groups.endLine) : undefined,
            column: sources[fileIndex] ? Number.parseInt(match.groups.col) : undefined,
            endColumn: sources[fileIndex] ? Number.parseInt(match.groups.endCol) : undefined,
          }
        : undefined
