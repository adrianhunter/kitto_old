...
import type { Argument, Arguments, AsmVarValues, DebugConfiguration, DebugLaunch, FileUri, LibraryEntity, StructEntity, SupportedParamType, TxContext, TypeResolver, VerifyError } from './internal.ts'
import {
  Bytes,
  ScryptType, SymbolType, arrayTypeAndSize, deduceGenericLibrary, deduceGenericStruct, getValidatedHexString, subArrayType, subscript,
} from './internal.ts'
import { isArrayType, isEmpty, isNode, path2uri, uri2path } from './utils.ts'
...
export function genLaunchConfigFile(resolver: TypeResolver, constructorArgs: Arguments, pubFuncArgs: Arguments,
  ...
  if (!isEmpty(asmArgs))
    Object.assign(debugConfig, { asmArgs })
