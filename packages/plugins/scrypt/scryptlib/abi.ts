import { bin2num } from './builtins.ts'
import type { ABIEntity } from './compilerWrapper.ts'
import { ABIEntityType } from './compilerWrapper.ts'
import type { AsmVarValues, TxContext, VerifyResult } from './contract.ts'
import { AbstractContract } from './contract.ts'
import { deserializeArgfromHex } from './deserializer.ts'
import { genLaunchConfigFile } from './launchConfig.ts'
import type { SupportedParamType, TypeResolver } from './scryptTypes.ts'
import { Int } from './scryptTypes.ts'
import { toScriptHex } from './serializer.ts'
import Stateful from './stateful.ts'
import { flatternArg } from './typeCheck.ts'
import { asm2int, bsv, buildContractCode, int2Asm } from './utils.ts'
...
export class ABICoder {
  ...
  encodeConstructorCall(contract: AbstractContract, hexTemplate: string, ...args: SupportedParamType[]): FunctionCall {
    ...
    const hasCodePartTemplate = !!hexTemplate.match(/<__codePart__>/g)
  ...
  encodeConstructorCallFromRawHex(contract: AbstractContract, hexTemplate: string, raw: string): FunctionCall {
    ...
    let dataPartInHex: string | undefined
    ...
    function checkOp(chunk: bsv.Script.IOpChunk) {
      ...
      if (Number.parseInt(op, 16) != chunk.opcodenum)
        throw err
    ...
    function checkPushData1(chunk: bsv.Script.IOpChunk) {
      ...
      if (Number.parseInt(next1Byte, 16) != chunk.len)
        throw err
  ...
  parseCallData(hex: string): CallData {
    ...
    let entity: ABIEntity | undefined
    if (pubFunCount === 1) {
      entity = pubFunAbis[0]
    }
    else {
    ...
    }).flat(Number.POSITIVE_INFINITY) as Arguments
