...
import type { Argument, LibraryEntity, ParamEntity } from './index.ts'
import { ABIEntityType, parseGenericType } from './index.ts'
import type { ContractEntity, OpCode, StaticEntity } from './compilerWrapper.ts'
import { getFullFilePath, loadSourceMapfromArtifact } from './compilerWrapper.ts'
import type { ABIEntity, AliasEntity, Arguments, FunctionCall, Script, StructEntity, TypeResolver } from './internal.ts'
import {
  ABICoder, CompileResult, DEFAULT_FLAGS, JSONParserSync, bsv, buildContractCode, findSrcInfoV1, findSrcInfoV2, hash160, isArrayType, path2uri, resolveType, subscript, uri2path,
} from './internal.ts'
import type { SupportedParamType, TypeInfo } from './scryptTypes.ts'
import { Bytes, Int, SymbolType, isScryptType } from './scryptTypes.ts'
import Stateful from './stateful.ts'
import { arrayTypeAndSize, checkSupportedParamType, flatternArg, hasGeneric, subArrayType } from './typeCheck.ts'
...
export interface AsmVarValues { [key: string]: string }
 ...
 */
  ...
  run_verify(unlockingScript: bsv.Script | string | undefined, txContext?: TxContext): VerifyResult {
    ...
    if (typeof unlockingScript === 'string')
      ...
      us = unlockingScript || new bsv.Script('')
    ...
    if ((bsi.errstr || '').includes('SCRIPT_ERR_NULLFAIL')) {
      if (!txCtx)
...
export function buildContractClass(artifact: Artifact | CompileResult): typeof AbstractContract {
  ...
  ContractClass.abi.forEach((entity: ABIEntity) => {
    ...
    if (!entity.name || invalidMethodName.includes(entity.name))
