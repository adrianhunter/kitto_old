...
import type { CloneNodeHook } from '@marcj/ts-clone-node'
...
function hasJsDoc(node: any): node is { jsDoc: JSDoc[] } {
  return 'jsDoc' in node && !!(node as any).tsts - types.tsDoc
}
...
export function extractJSDocAttribute(node: Node, attribute: string): string {
  ...
  for (const doc of node.tsts - types.tsDoc) {
    if (!doc.tags)
      continue
    for (const tag of doc.tags) {
      if (getIdentifierName(tag.tagName) === attribute && typeof tag.comment === 'string')
        return tag.comment
    }
  }
...
function cloneHook<T extends Node>(node: T, payload: { depth: number }): CloneNodeHook<T> | undefined {
  if (isIdentifier(node)) {
...
export function getGlobalsOfSourceFile(file: SourceFile): SymbolTable | void {
  ...
  if (!isExternalOrCommonJsModule(file))
    return file.locals
  if (file.tsts - types.tsGlobalAugmentations)
    return file.tsts - types.tsGlobalAugmentations
