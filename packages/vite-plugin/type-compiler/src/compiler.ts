...
import { MappedModifier, ReflectionOp, TypeNumberBrand } from '../../type-spec/src/type.ts'
import type { SourceFile } from './ts-types.ts'
import {
  NodeConverter,
...
} from './reflection-ast.ts'
import type { PackExpression } from './reflection-ast.ts'
...
export function debugPackStruct(sourceFile: SourceFile, forType: Node, pack: { ops: ReflectionOp[]; stack: PackExpression[] }): void {
  ...
  for (const s of pack.stack) {
    if (typeof s === 'object' && 'getText' in s)
...
export class ReflectionTransformer implements CustomTransformer {
  ...
  transformSourceFile(sourceFile: SourceFile): SourceFile {
    ...
    if ((sourceFile as any).deepkitTransformed)
      return sourceFile;
    ...
    if (typeof this.compilerOptions.configFilePath === 'string') {
    ...
    if (basePath) {
      ...
      if (!this.reflectionMode && currentConfig.reflection !== undefined)
      ...
      while ((this.reflectionMode === undefined || this.compilerOptions === undefined) && typeof basePath === 'string' && currentConfig.extends) {
        ...
        if (!nextConfig)
          break
        if (!this.reflectionMode && nextConfig.config.reflection !== undefined)
  ...
  protected resolveImportSpecifier(declarationName: __String, importOrExport: ExportDeclaration | ImportDeclaration, sourceFile: SourceFile): Declaration | undefined {
    if (!importOrExport.moduleSpecifier)
      return
    if (!isStringLiteral(importOrExport.moduleSpecifier))
      return
  ...
  protected findReflectionConfig(node: Node, program?: CompilerProgram): ReflectionConfig {
    ...
    do {
      ...
      for (const tag of tags) {
        if (!reflection && getIdentifierName(tag.tagName) === 'reflection' && typeof tag.comment === 'string')
    ...
    if (this.reflectionMode !== undefined)
