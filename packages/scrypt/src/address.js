...
class Address {
  constructor(data, network, type) {
    ...
    info.network = info.network || Networks.get(network) ||
  ...
  _classifyArguments(data, network, type) {
    ...
    if (
      (data instanceof Buffer || data instanceof Uint8Array) && data.length === 20
    ) {
      return Address._transformHash(data);
    } else if (
      (data instanceof Buffer || data instanceof Uint8Array) && data.length === 21
    ) {
