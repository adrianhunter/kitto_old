...
class Interpreter {
   ...
   */
  verify(scriptSig, scriptPubkey, tx, nin, flags, satoshisBN, sighashScript) {
    ...
    if (
      (flags & Interpreter.SCRIPT_VERIFY_P2SH) && scriptPubkey.isScriptHashOut()
    ) {
  ...
  set(obj) {
    ...
    this.pbegincodehash = typeof obj.pbegincodehash !== "undefined"
      ? obj.pbegincodehash
      : this.pbegincodehash;
    this.nOpCount = typeof obj.nOpCount !== "undefined"
      ? obj.nOpCount
      : this.nOpCount;
  ...
  checkSignatureEncoding(buf) {
    ...
    if (
      (this.flags &
          (Interpreter.SCRIPT_VERIFY_DERSIG | Interpreter.SCRIPT_VERIFY_LOW_S |
            Interpreter.SCRIPT_VERIFY_STRICTENC)) !== 0 && !Signature.isTxDER(buf)
    ) {
   ...
   * @param {*} stack
   */
  _callbackStack(stack, pc, scriptType) {
    if (typeof this.stackListener === "function") {
      try {
        ...
        console.error(
          `Error: ${err} in _updateStack pc: ${pc}, opcode ${
            Opcode.fromNumber(chunk.opcodenum).toSafeString()
          }`,
        );
      }
    }
  ...
  checkSequence(nSequence) {
    ...
    var nLockTimeMask = Interpreter.SEQUENCE_LOCKTIME_TYPE_FLAG |
  ...
  step(scriptType) {
    ...
    if (
      opcodenum > Opcode.OP_16 && ++this.nOpCount > Interpreter.MAX_OPCODE_COUNT
    ) {
    ...
    if (fExec && opcodenum >= 0 && opcodenum <= Opcode.OP_PUSHDATA4) {
      ...
      fExec || (Opcode.OP_IF <= opcodenum && opcodenum <= Opcode.OP_ENDIF)
    ) {
      switch (opcodenum) {
        // Push value
        ...
        case Opcode.OP_ELSE:
          ...
          this.vfExec[this.vfExec.length - 1] = !this
            .vfExec[this.vfExec.length - 1];
          break;
        ...
        case Opcode.OP_EQUALVERIFY:
          ...
          this.stack.pop();
          this.stack.push(
            fEqual ? Interpreter.getTrue() : Interpreter.getFalse(),
          );
        ...
        case Opcode.OP_WITHIN:
          ...
          this.stack.pop();
          this.stack.push(
            fValue ? Interpreter.getTrue() : Interpreter.getFalse(),
          );
          break;
        ...
        case Opcode.OP_CHECKSIGVERIFY:
          ...
          if (
            !fSuccess && (this.flags & Interpreter.SCRIPT_VERIFY_NULLFAIL) &&
            bufSig.length
          ) {
          ...
          this.stack.push(
            fSuccess ? Interpreter.getTrue() : Interpreter.getFalse(),
          );
        ...
        case Opcode.OP_CHECKMULTISIGVERIFY:
          ...
          var nKeysCount = BN.fromScriptNumBuffer(stacktop(-i), fRequireMinimal)
            .toNumber();
          ...
          var nSigsCount = BN.fromScriptNumBuffer(stacktop(-i), fRequireMinimal)
            .toNumber();
        ...
        case Opcode.OP_SPLIT:
          ...
          var position = BN.fromScriptNumBuffer(stacktop(-1), fRequireMinimal)
            .toNumber();
        ...
        case Opcode.OP_NUM2BIN:
          ...
          var size = BN.fromScriptNumBuffer(stacktop(-1), fRequireMinimal)
            .toNumber();
...
Interpreter.DEFAULT_FLAGS =
  ...
  Interpreter.SCRIPT_ENABLE_MONOLITH_OPCODES | // TODO: to be removed after upgrade to bsv 2.0
  Interpreter.SCRIPT_VERIFY_STRICTENC |
  Interpreter.SCRIPT_ENABLE_SIGHASH_FORKID | Interpreter.SCRIPT_VERIFY_LOW_S |
