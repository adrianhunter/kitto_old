...
class HDPrivateKey {
  constructor(arg) {
    ...
    if (Network.get(arg)) {
      ...
      if (HDPrivateKey.isValidSerialized(arg)) {
        ...
        Buffer.isBuffer(arg) && HDPrivateKey.isValidSerialized(arg.toString())
      ) {
  ...
  _deriveWithNumber(index, hardened, nonCompliant) {
    ...
    var privateKey = leftPart.add(this.privateKey.toBigNumber()).umod(
      Point.getN(),
    ).toBuffer({
      size: 32,
    });
  ...
  static _validateBufferArguments(arg) {
    var checkBuffer = function (name, size) {
      ...
      assert(
        buff.length === size,
        name + " has not the expected size: found " + buff.length +
          ", expected " + size,
      );
    };
 ...
 *  @return {Object}
 */
HDPrivateKey.prototype.toObject =
  HDPrivateKey.prototype.toJSON =
    function toObject() {
      return {
        network:
          Network.get(this._buffers.version.readUInt32BE(0), "xprivkey").name,
...
HDPrivateKey.ChildIndexEnd = HDPrivateKey.ChildIndexStart +
  HDPrivateKey.ChildIndexSize;
HDPrivateKey.ChainCodeStart = HDPrivateKey.ChildIndexEnd;
HDPrivateKey.ChainCodeEnd = HDPrivateKey.ChainCodeStart +
  HDPrivateKey.ChainCodeSize;
HDPrivateKey.PrivateKeyStart = HDPrivateKey.ChainCodeEnd + 1;
HDPrivateKey.PrivateKeyEnd = HDPrivateKey.PrivateKeyStart +
  HDPrivateKey.PrivateKeySize;
HDPrivateKey.ChecksumStart = HDPrivateKey.PrivateKeyEnd;
HDPrivateKey.ChecksumEnd = HDPrivateKey.ChecksumStart +
  HDPrivateKey.CheckSumSize;
