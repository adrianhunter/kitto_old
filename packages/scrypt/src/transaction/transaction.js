...
class Transaction {
  ...
  _hasDustOutputs(opts) {
    ...
    for (index in this.outputs) {
      ...
      if (
        output.satoshis < Transaction.DUST_AMOUNT && !output.script.isDataOut() &&
  ...
  fromObject(arg) {
    ...
    _.each(transaction.inputs, function (input) {
      ...
      if (script.isPublicKeyHashOut()) {
        ...
        script.isScriptHashOut() && input.publicKeys && input.threshold
      ) {
  ...
  from(utxo, pubkeys, threshold) {
    ...
    var exists = _.some(this.inputs, function (input) {
      ...
      return input.prevTxId.toString("hex") === utxo.txId &&
  ...
  addInput(input, outputScript, satoshis) {
    ...
    if (
      !input.output && (_.isUndefined(outputScript) || _.isUndefined(satoshis))
    ) {
    ...
    if (!input.output && outputScript && !_.isUndefined(satoshis)) {
      outputScript = outputScript instanceof Script
        ? outputScript
  ...
  removeInput(txId, outputIndex) {
    ...
    if (!outputIndex && _.isNumber(txId)) {
      ...
      index = _.findIndex(this.inputs, function (input) {
        return input.prevTxId.toString("hex") === txId &&
   ...
   */
  setInputScript(options, unlockScriptOrCallback) {
    var inputIndex = 0;
    var privateKey;
    var sigtype;
    var isLowS = false;
   ...
   */
  async setInputScriptAsync(options, unlockScriptOrCallback) {
    var inputIndex = 0;
    var sigtype;
    var isLowS = false;
    if (typeof options === "number") {
    ...
    if (unlockScriptOrCallback instanceof Function) {
      ...
      throw new errors.InvalidArgument(
        "Must provide a callback returns a unlocking script",
      );
    }
   ...
   * @returns output
   */
  setOutput(outputIndex, outputOrcb) {
  ...
  async sealAsync() {
    ...
    this._inputsMap.forEach(function (options, key) {
      ...
      promises.push(
        Promise.resolve(options.callback(self, outputInPrevTx)).then(
          (unlockScript) => {
  ...
  checkFeeRate(feePerKb) {
    ...
    var estimatedSize = this._estimateSize();
    var expectedRate = (feePerKb || this._feePerKb || Transaction.FEE_PER_KB) /
      1000;
  ...
  getPreimage(inputIndex, sigtype, isLowS) {
    $.checkArgumentType(inputIndex, "number", "inputIndex");
    sigtype = sigtype || (Signature.SIGHASH_ALL | Signature.SIGHASH_FORKID);
    isLowS = isLowS || false;
    inputIndex = inputIndex || 0;
    var preimage = this.inputs[inputIndex].getPreimage(
      this,
      inputIndex,
      sigtype,
      isLowS,
  ...
  getSignature(inputIndex, privateKeys, sigtypes) {
    $.checkArgumentType(inputIndex, "number", "inputIndex");
    var results = [];
    ...
    if (privateKeys) {
      ...
      var sigs = this.inputs[inputIndex].getSignatures(
        this,
        privateKeys,
        inputIndex,
        sigtypes,
  ...
  addInputFromPrevTx(prevTx, outputIndex) {
...
var hashProperty = {
  ...
  enumerable: true,
  get: function () {
    this._hash = new BufferReader(this._getHash()).readReverse().toString(
      "hex",
    );
    return this._hash;
  },
...
Transaction.prototype.uncheckedSerialize =
  Transaction.prototype.toString =
    function () {
      return this.toBuffer().toString("hex");
    };

Transaction.prototype.toObject =
  Transaction.prototype.toJSON =
    function toObject() {
      var inputs = [];
