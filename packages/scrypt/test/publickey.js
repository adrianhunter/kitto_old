...
describe('PublicKey', () => {
  ...
  describe('validating errors on creation', () => {
    it('errors if data is missing', () => {
      ((() => new PublicKey())).should.throw('First argument is required, please include public key data.')
    })

    it('errors if an invalid point is provided', () => {
      ((() => new PublicKey(invalidPoint))).should.throw('Point does not lie on the curve')
    })

    it('errors if a point not on the secp256k1 curve is provided', () => {
      ((() => new PublicKey(new Point(1000, 1000)))).should.throw('Point does not lie on the curve')
    })

    it('errors if the argument is of an unrecognized type', () => {
      ((() => new PublicKey(new Error()))).should.throw('First argument is an unrecognized data format.')
    })
  })
  ...
  describe('#fromX', () => {
    ...
    it('should error because odd was not included as a param', () => {
      const x = BN.fromBuffer(Buffer.from('1ff0fe0f7b15ffaa85ff9f4744d539139c252a49710fb053bb9f2b933173ff9a', 'hex'));
      ((() => PublicKey.fromX(null, x))).should.throw('Must specify whether y is odd or not (true or false)')
    })
  })
  ...
  describe('#validate', () => {
    ...
    it('should throw an error if pubkey is invalid', () => {
      ...
      ((() => PublicKey.fromString(hex))).should.throw('Invalid y value for curve.')
    })
    ...
    it('should throw an error if pubkey is infinity', () => {
      ((() => new PublicKey(Point.getG().mul(Point.getN())))).should.throw('Point cannot be equal to Infinity')
    })
  })
})
