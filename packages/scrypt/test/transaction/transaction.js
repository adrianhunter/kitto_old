...
describe('Transaction', () => {
  it('should be able to add two outputs with short addresses', () => {
    let errors = 0;
    try {
  ...
  describe('adding inputs', () => {
    ...
    it('adds OP_FALSE in front of OP_RETURN', () => {
      const transaction = new Transaction()
        .from(simpleUtxoWith100000Satoshis)
    ...
    describe('skipping checks', () => {
      ...
      it('can skip the check for too much fee', buildSkipTest(
        transaction => transaction
          .fee(50000000)
          .change(changeAddress)
          .sign(privateKey), 'disableLargeFees', errors.Transaction.FeeError.TooLarge
      ))
      it('can skip the check that prevents dust outputs', buildSkipTest(
        transaction => transaction
          .to(toAddress, 100)
          .change(changeAddress)
          .sign(privateKey), 'disableDustOutputs', errors.Transaction.DustOutputs
      ))
      it('can skip the check that prevents unsigned outputs', buildSkipTest(
        transaction => transaction
          .to(toAddress, 10000)
          .change(changeAddress), 'disableIsFullySigned', errors.Transaction.MissingSignatures
      ))
      it('can skip the check that avoids spending more bitcoins than the inputs for a transaction', buildSkipTest(
        transaction => transaction
          .to(toAddress, 10000000000000)
          .change(changeAddress)
          .sign(privateKey), 'disableMoreOutputThanInput', errors.Transaction.InvalidOutputAmountSum
      ))
    })
  })
  ...
  describe('inputAmount + outputAmount', () => {
    ...
    it('returns correct values for coinjoin transaction', () => {
      ...
      expect(() => {
        const ia = transaction.inputAmount; //eslint-disable-line
  ...
  describe('output ordering', () => {
    ...
    it('allows the user to sort outputs according to a criteria', () => {
      const sorting = array => [array[3], array[2], array[1], array[0]];
    ...
    it('fails if the provided function does not work as expected', () => {
      const sorting = array => [array[0], array[1], array[2]];
      expect(() => {
        transaction.sortOutputs(sorting)
  ...
  describe('BIP69 Sorting', () => {
    ...
    describe('bitcoinjs fixtures', () => {
      ...
      const getIndexOrder = (original, sorted) => sorted.map(value => original.indexOf(value));
      ...
      fixture.outputs.forEach(outputSet => {
        it(outputSet.description, () => {
          const tx = new Transaction();
          outputSet.outputs = outputSet.outputs.map(({script, value}) => new Output({
