...
describe("Signature", () => {
  ...
  describe("#toTxFormat", () => {
    it("should parse this known signature and rebuild it with updated zero-padded sighash types", () => {
      ...
      sig.toTxFormat().toString("hex").should.equal(
        `${original.slice(0, -2)}81`,
      );
      sig.set({
        nhashtype: Signature.SIGHASH_SINGLE,
      });
      sig.toTxFormat().toString("hex").should.equal(
        `${original.slice(0, -2)}03`,
      );
    });
  });
