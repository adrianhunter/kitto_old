import ECDSA from "bsv/crypto/ecdsa.js";
import Hash from "bsv/crypto/hash.js";
import Privkey from "bsv/privatekey.js";
import Pubkey from "bsv/publickey.js";
import Signature from "bsv/crypto/signature.js";
import BN from "bsv/crypto/bn.js";
import point from "bsv/crypto/point.js";
const should = require("chai").should();
import vectors from "data/ecds";
...
describe("ECDSA", () => {
  ...
  describe("#sign", () => {
    ...
    it("should generate right K", () => {
      ...
      const signature1 = ECDSA.sign(msg1, Privkey.fromBuffer(pk)).toBuffer()
        .toString("hex");
  ...
  describe("signing and verification", () => {
    describe("@sign", () => {
      ...
      it("should produce a signaturei", () => {
        const sig = ECDSA.signWithCalcI(ecdsa.hashbuf, ecdsa.privkey);
        (sig instanceof Signature).should.equal(true);
        should.exist(sig.i);
        sig.toCompact().toString("base64").should.equal(
    ...
    describe("vectors", () => {
      ...
      vectors.invalid.sigError.forEach(
        ({ description, signature, message, exception }, i) => {
          it(`should validate invalid.sigError vector ${i}: ${description}`, () => {
            const ecdsa = ECDSA().set({
              pubkey: Pubkey.fromPoint(point.fromX(true, 1)),
              sig: new Signature(new BN(signature.r), new BN(signature.s)),
              hashbuf: Hash.sha256(Buffer.from(message)),
            });
            ecdsa.sigError().should.equal(exception);
          });
        },
      );
