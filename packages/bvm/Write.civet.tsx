...
export const writeVarint = (writer: BufferWriter, n: number) => {
...
export const writeU64LE = (writer: BufferWriter, n: number) => { 
  if (n > Number.MAX_SAFE_INTEGER) {
...
export const writeU32LE = (writer: BufferWriter, n: number) => {
  if (n > 0xffffffff) {
    throw new Error("number too large")
  }
...
export const writeTx = (writer: BufferWriter, tx: Tx) => {
...
export function writeDER (
  ...
  signature: {
    ...
    //@ts-ignore
  const r = signature.r[0] & 0x80
    ? [0x00].concat(Array.from(signature.r))
    : signature.r
    //@ts-ignore
  const s = signature.s[0] & 0x80
  ...
  //@ts-ignore
  der.set(s, rlength + 6)
  return writer.write(der)
}


describe("Write writeVarint", () => { 
  it('valid', () => { 
    const testValid = (n: number, enc: Buf) => { 
      return assert.equal(Array.from(writeVarint(new BufferWriter(), n).toBuffer()), enc)
    }
    testValid(0, [0])
    testValid(1, [1])
    testValid(252, [252])
    ...
    return testValid(Number.MAX_SAFE_INTEGER, [0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1f, 0x00])
  })
  return it('throws if too big', () => { 
    assert.throws(() => writeVarint(new BufferWriter(), Number.MAX_SAFE_INTEGER + 1), Error, 'varint too large')
    return assert.throws(() => writeVarint(new BufferWriter(), Number.MAX_VALUE), Error, 'varint too large')
  })
})
...
describe('Write writeU64LE', () => { 
  it('valid', () => { 
    ...
    return assert.equal(Array.from(writeU64LE(new BufferWriter(), Number.MAX_SAFE_INTEGER).toBuffer()), [0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1f, 0x00])
  })
  it('multiple', () => { 
    const bw = new BufferWriter()
    writeU64LE(bw, 0)
    writeU64LE(bw, Number.MAX_SAFE_INTEGER)
    return assert.equal(Array.from(bw.toBuffer()), [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x1f, 0x00])
  })
  return it('throws if too big', () => {
    return assert.throws(() => writeU64LE(new BufferWriter(), Number.MAX_SAFE_INTEGER + 1), Error, 'number too large')
  })
})
...
describe('Write writeU32LE', () => { 
  it('valid', () => { 
    ...
    return assert.equal(Array.from(writeU32LE(new BufferWriter(), 0xffffffff).toBuffer()), [0xff, 0xff, 0xff, 0xff])
  })
  it('multiple', () => {
    const bw = new BufferWriter()
    writeU32LE(bw, 0x00000000)
    writeU32LE(bw, 0xffffffff)
    return assert.equal(Array.from(bw.toBuffer()), [0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff])
  })
  return it('throws if too big', () => {
    return assert.throws(() => writeU32LE(new BufferWriter(), 0xffffffff + 1), Error, 'number too large')
  })
})

describe('Write writePushData', () => {
  return it('valid', () => { 
    function testValid (x: Buf, y: Buf) {
      const actual = Array.from(writePushData(new BufferWriter(), x).toBuffer())
      return assert.equal(Array.from(actual), y)
    }
    testValid([], [0])
    testValid([0], [1, 0])
    testValid([0, 0], [2, 0, 0])
    ...
    return testValid(new Array(0xFFFF + 1).fill(0), [78, 0, 0, 1, 0].concat(new Array(0xFFFF + 1).fill(0)))
  })
})
