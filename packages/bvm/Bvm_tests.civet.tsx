...
import {run} from "./Bvm.civet";
// {evalScript} from ./bvm
// run := evalScript
import { Tx } from "./Tx.civet";
import {encodePushData} from "./Encode.civet";
import {decodeHex, decodeTx} from "./Decode.civet";
import { Op } from "./Op"

describe("Bvm", function() {
...
// //     // assert.isTrue(vm.stackTrace.every(l => l.length === 3))
  ...
  it('valid script returns vm omitting stack trace', async () => { 
    const vm = await run([Op._2, Op._3], [Op.add, Op._5, Op.equal], undefined, undefined, undefined, { trace: false })

    assert.equal(vm.success, true)
    return assert.equal(vm.error, null)
  })
...
// //   })
  it('valid script passes', async () => { 
    async function pass (script: any) {
      console.log(`run: ${JSON.stringify(script)}`)
      const vm2 = await run([], script, undefined, undefined, undefined, { async: true })
      // console.log vm2
      assert.equal(vm2.success, true, `run: ${JSON.stringify(script)} stack: ${JSON.stringify(vm2, null, 2)}`)
      return assert.equal(vm2.error, null)
    }
    const tests = [
      [Op.true],
      [Op._1],
...
//   })
  ...
  return it('bad', async () => { 
    async function fail (script: any) {
      return assert.equal((await run([], script, undefined, undefined, undefined)).success, false)
    }
